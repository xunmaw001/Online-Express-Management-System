            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑快递</li>
                    </ol>
                </div>
            </div>
        </div>
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="kuaidiMoney" name="kuaidiMoney" class="form-control"
                                               onchange="kuaidiMoneyChickValue(this)"  placeholder="快递金额">
                                    </div>

                                    <div class="form-group  col-md-6">
                                        <label>快递详细内容</label>
                                        <input id="kuaidiContentupload" name="file" type="file">
                                        <script id="kuaidiContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var kuaidiContentUe = UE.getEditor('kuaidiContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="kuaidiContent" id="kuaidiContent-input">
                                    </div>
                                    <div class="form-group col-md-6">
...
<script>
    ...
    function upload() {
        ...
        $('#kuaidiContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('kuaidiContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.kuaidiContent != null && ruleForm.kuaidiContent != 'null' && ruleForm.kuaidiContent != '' && $("#kuaidiContentupload").length>0) {

            var kuaidiContentUeditor = UE.getEditor('kuaidiContentEditor');
            kuaidiContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.kuaidiContent != null){
                    mes = ''+ ruleForm.kuaidiContent;
                }
                kuaidiContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#kuaidiContentupload").length>0) {
            var kuaidiContentEditor = UE.getEditor('kuaidiContentEditor');
            if (kuaidiContentEditor.hasContents()) {
                $('#kuaidiContent-input').attr('value', kuaidiContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yonghuId: "required",
                kuaidiNumber: "required",
                kuaidiName: "required",
                kuaidiMoney: "required",
                kuaidiContent: "required",
                // kuaidiThisValue: "required",
                // kuaidiTypes: "required",
                insertTime: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                kuaidiNumber: "快递单号不能为空",
                kuaidiName: "运送物品不能为空",
                kuaidiMoney: "快递金额不能为空",
                kuaidiContent: "快递详细内容不能为空",
                // kuaidiThisValue: "快递最新物流状态不能为空",
                // kuaidiTypes: "是否签收不能为空",
                insertTime: "快递时间不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addkuaidi != null && addkuaidi != "" && addkuaidi != "null") {
            ...
            $('#submitBtn').text('新增');
            var date = new Date();
            var number = date.getTime()+ Math.floor(Math.random()*10);//生成快递编号
            $("#kuaidiNumber").val(number);

            $("#kuaidiThisValue").val("入库");//添加物流状态
